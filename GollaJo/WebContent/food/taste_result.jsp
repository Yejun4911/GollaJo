...
function reresult() {
	arr.shift();
	if(arr.length!=0){
		document.getElementById("situation").innerHTML=arr[0];
		//document.getElementById("reset1").setAttribute("onClick", url());
	
	}else{
		document.getElementById("situation").innerHTML="메뉴가 더 이상 없습니다.";
		document.getElementById("reset1").value="처음으로";
		document.getElementById("reset1").setAttribute("onClick", "location.href='${pageContext.request.contextPath}/food/food_taste1.jsp?choice=1'");
	}
}
function url(){
	location.href="${pageContext.request.contextPath}/food/taste_real_result.jsp?confirm="+arr[0];
}
</script>
</head>
        ...
        <div class='content center'>
            <h1 id='master'>
                ...
                <div class='master-container-scroller_item'>
				 <a class='cta-link' value="${list[0]}" id="situation">${list[0]}</a>.
				</div>
                </div>
				 <div id="howabout">어떤가요?</div>
            </h1>
            <div>
            <button id='resultbtn' name="reset" value="재선택" id="again" class="again" onclick="reresult();"> 재선택 </button>
		  	<button id='confirmbtn' name="confirm" value="선택" id="confirm" class="confirm" onclick="url();"> 선택 </button>
			</div>
        </div>
    </div>
</section>
